...
<?php 
	...
	<?= $this->session->flashdata('notif',true) ?>
		<form action="<?= base_url('master_barang/model/simpan') ?>" method="post">
			<div>
				<label for="crud-form-1" class="form-label">Name Model</label>
				<input id="crud-form-1" type="text" class="form-control w-full" name="nama_model" placeholder="Input text">
			</div>
		...
		<table id="example" class="table table-report -mt-2">
			...
			<tbody>
				...
				<tr class="intro-x">
					...
					<td>
						<a href="" class="font-medium whitespace-nowrap"><?= $mm['model'] ?></a>
					</td>
					...
					<td class="table-report__action w-56">
						<div class="flex justify-center items-center">
							...
							<a class="flex items-center mr-3" href="javascript:;" data-tw-toggle="modal"
								data-tw-target="#edit-modal<?= $mm['id_model'] ?>"> <i data-lucide="check-square"
									class="w-4 h-4 mr-1"></i> Edit </a>

							<a class="flex items-center text-danger"
								href="<?php echo site_url('master_barang/model/delete_data/'.$mm['id_model']); ?>" data-tw-toggle="modal"
					...
					<div id="edit-modal<?= $mm['id_model'] ?>" class="modal" tabindex="-1" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
									<form action="<?= base_url('master_barang/model/update') ?>" method="POST">
										<input type="hidden" name="id_model" value="<?= $mm['id_model'] ?>">
										<div class="col-span-12 sm:col-span-12"> <label for="modal-form-1"
												class="form-label">Nama</label>
											<input id="modal-form-1" type="text" class="form-control"
												placeholder="example@gmail.com" name="nama_model" value="<?= $mm['model'] ?>">
										</div>
