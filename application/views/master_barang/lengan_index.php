<div class="content content--top-nav"> 	<!-- BEGIN: Form Layout -->
	...
	<?= $this->session->flashdata('notif',true) ?>
		<form action="<?= base_url('master_barang/lengan/simpan') ?>" method="post">
			<div>
				<label for="crud-form-1" class="form-label">Name Size</label>
				<input id="crud-form-1" type="text" class="form-control w-full" name="nama_lengan" placeholder="Input text">
			</div>
	...
	<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
		<table id="example" class="table table-report -mt-2">
			...
			<tbody>
				<?php foreach($lengan as $ll) { ?>

				<tr class="intro-x">

					<td>
						<a href="" class="font-medium whitespace-nowrap"><?= $ll['lengan'] ?></a>
					</td>
					...
					<td class="table-report__action w-56">
						<div class="flex justify-center items-center">
							...
							<a class="flex items-center mr-3" href="javascript:;" data-tw-toggle="modal"
								data-tw-target="#edit-modal<?= $ll['id_lengan'] ?>"> <i data-lucide="check-square"
									class="w-4 h-4 mr-1"></i> Edit </a>

							<a class="flex items-center text-danger"
								href="<?php echo site_url('master_barang/lengan/delete_data/'.$ll['id_lengan']); ?>" data-tw-toggle="modal"
					...
					<div id="edit-modal<?= $ll['id_lengan'] ?>" class="modal" tabindex="-1" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
									<form action="<?= base_url('master_barang/lengan/update') ?>" method="POST">
										<input type="hidden" name="id_lengan" value="<?= $ll['id_lengan'] ?>">
										<div class="col-span-12 sm:col-span-12"> <label for="modal-form-1"
												class="form-label">Nama</label>
											<input id="modal-form-1" type="text" class="form-control"
												placeholder="example@gmail.com" name="nama_lengan" value="<?= $ll['lengan'] ?>">
										</div>
