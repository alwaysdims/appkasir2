<div class="content content--top-nav"><!-- BEGIN: Form Layout -->
	...
	<?= $this->session->flashdata('notif',true) ?>
		<form action="<?= base_url('master_barang/size/simpan') ?>" method="post">
			<div>
				...
				<input id="crud-form-1" type="text" class="form-control w-full" name="nama_size" placeholder="Input text">
			</div>
	...
	<div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
		<table class="table table-report -mt-2">
			...
			<tbody>
				<?php foreach($size as $ss) { ?>

				<tr class="intro-x">

					<td>
						<a href="" class="font-medium whitespace-nowrap"><?= $ss['size'] ?></a>
					</td>
					...
					<td class="table-report__action w-56">
						<div class="flex justify-center items-center">
							...
							<a class="flex items-center mr-3" href="javascript:;" data-tw-toggle="modal"
								data-tw-target="#edit-modal<?= $ss['id_size'] ?>"> <i data-lucide="check-square"
									class="w-4 h-4 mr-1"></i> Edit </a>

							<a class="flex items-center text-danger"
								href="<?php echo site_url('master_barang/size/delete_data/'.$ss['id_size']); ?>" data-tw-toggle="modal"
					...
					<div id="edit-modal<?= $ss['id_size'] ?>" class="modal" tabindex="-1" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
									<form action="<?= base_url('master_barang/size/update') ?>" method="POST">
										<input type="hidden" name="id_size" value="<?= $ss['id_size'] ?>">
										<div class="col-span-12 sm:col-span-12"> <label for="modal-form-1"
												class="form-label">Nama</label>
											<input id="modal-form-1" type="text" class="form-control"
												placeholder="example@gmail.com" name="nama_size" value="<?= $ss['size'] ?>">
										</div>
